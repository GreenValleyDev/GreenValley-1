...
-----------------------------------------------------------------------------------------------------------------------------------------
psRP = {}
Tunnel.bindInterface("ps_groups",psRP)
vCLIENT = Tunnel.getInterface("ps_groups")
-----------------------------------------------------------------------------------------------------------------------------------------
-- QUERYES
-----------------------------------------------------------------------------------------------------------------------------------------
DB.prepare("ps_groups/get_all_datatables","SELECT * FROM vrp_user_data WHERE dkey = 'vRP:datatable'")
DB.prepare("ps_groups/get_user_datatables","SELECT * FROM vrp_user_data WHERE dkey = 'vRP:datatable' AND user_id = @user_id")
DB.prepare("ps_groups/get_blacklist","SELECT * FROM ps_groups_blacklist WHERE user_id = @user_id")
DB.prepare("ps_groups/add_blacklist","INSERT INTO ps_groups_blacklist (user_id, time) VALUES (@user_id, @time)")
DB.prepare("ps_groups/edit_blacklist","UPDATE ps_groups_blacklist SET time = @time WHERE user_id = @user_id")
-----------------------------------------------------------------------------------------------------------------------------------------
-- VARIABLES
-----------------------------------------------------------------------------------------------------------------------------------------
local groupslist = {}
	...
	if user_id then
        ...
        if check then
            ...
            if groups[organization] and groups[organization].permissions[permissionid] then
                ...
                if bank then
                    local money = getServerData("ps_groups:"..organization..":bank") or 0
    ...
    if user_id then
		...
		if groups[organization] then
			...
			if blacklist.active then
				...
				local getblacklist = DB.query("ps_groups/get_blacklist", { user_id = nuser_id })
    ...
    if user_id then
		...
		if groups[organization] then
			...
			if blacklist.active then
				...
				if #getblacklist > 0 then
					DB.execute("ps_groups/edit_blacklist", { user_id = nuser_id, time = time })
				else
					DB.execute("ps_groups/add_blacklist", { user_id = nuser_id, time = time })
				end
...
function psRP.getOrgChestLogs(data)
    ...
    if user_id then
		...
		local nlogs = getServerData("ps_groups:"..organization..":chestlogs") or ""
...
function psRP.getOrgBankLogs(data)
    ...
    if user_id then
		...
		local nlogs = getServerData("ps_groups:"..organization..":banklogs") or ""
		nlogs = json.decode(nlogs)
    ...
    if user_id then
		...
		if amount > 0 then
			if bank > amount then
				...
				local money = numberToInt(getServerData("ps_groups:"..organization..":bank")) or 0
				local total = money + amount

				setServerData("ps_groups:"..organization..":bank", total)
				...
				setServerData("ps_groups:"..organization..":banklogs", logs)
...
function psRP.remOrgBank(data)
    ...
    if user_id then
		local organization = data.organization
		local amount       = numberToInt(data.amount)
		local bank         = numberToInt(getServerData("ps_groups:"..organization..":bank")) or 0
		local currency     = config.currency
...
-- ps_groups:SetClothe
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterServerEvent("ps_groups:chestlogs",function(user_id, chest, item, amount, typelog)
	local source = source
	if user_id and chest and item and amount and typelog then
		...
		if organization ~= nil then
			if typelog == "up" then
				...
				setServerData("ps_groups:"..organization..":chestlogs", logs)
			end
	...
	if user_id then
		...
		if check and args[1] ~= nil and args[2] ~= nil then
			if args[1] == "add" and args[3] ~= nil then
				...
				local getblacklist = DB.query("ps_groups/get_blacklist", { user_id = tonumber(args[2]) })

				if #getblacklist > 0 then
					DB.execute("ps_groups/edit_blacklist", { user_id = tonumber(args[2]), time = time })
				else
					DB.execute("ps_groups/add_blacklist", { user_id = tonumber(args[2]), time = time })
				end
...
Citizen.CreateThread(function()
    if GetCurrentResourceName() == "ps_groups" then

		local datatables = DB.query("ps_groups/get_all_datatables")
