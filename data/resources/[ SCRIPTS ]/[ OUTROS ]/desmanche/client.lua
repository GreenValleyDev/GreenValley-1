...
---------------------------------------------------------------------
-- ELITECAR
local IniciarServico = {1365.96,-708.79,68.21} -- Onde se inicia o serviço e verifica a existência de um carro
local LocalDesmancharCarro = {1375.14,-715.91,67.54} -- Onde deve haver o carro que será desmanchado para poder continuar o desmanche
local LocalFerramentas = {1370.64, -716.94, 68.21} -- Local onde 'pegará' as ferramentas
local AnuncioChassi = {1369.1, -707.61, 68.21} -- Onde finalizará a missão para entregar o chassi e receber dinheiro e itens
local Computador = {1, 1, 1} -- Local onde ficará o computador de venda

-- DRIFT KING
local IniciarServico2 = {-333.17, -144.52, 39.01} -- Onde se inicia o serviço e verifica a existência de um carro
local LocalDesmancharCarro2 = {-326.82, -144.78, 38.64} -- Onde deve haver o carro que será desmanchado para poder continuar o desmanche
local LocalFerramentas2 = {-319.77, -131.88, 38.98} -- Local onde 'pegará' as ferramentas
local AnuncioChassi2 = {-323.65, -129.51, 39.01} -- Onde finalizará a missão para entregar o chassi e receber dinheiro e itens
local Computador2 = {1, 1, 1} -- Local onde ficará o computador de venda
---------------------------------------------------------------------
--VARIAVEIS
---------------------------------------------------------------------
local blips = {}
...
---------------------------------------------------------------------
-- DESMANCHE DRIFT KING DK
---------------------------------------------------------------------


Citizen.CreateThread(function() 

    lepitopi = CreateObject(GetHashKey("prop_laptop_lester"),Computador2[1], Computador2[2], Computador2[3]-0.97,true,true,true)
    SetEntityHeading(lepitopi, Computador2[4])
    ...
    while true do
        ...
        if permitido then
            ...
            local pedCoords = GetEntityCoords(ped)
            if etapa2 == 0 then
                local dist = Vdist(pedCoords, IniciarServico2[1], IniciarServico2[2], IniciarServico2[3])
                if dist < 10 then
                    DrawMarker(21, IniciarServico2[1], IniciarServico2[2], IniciarServico2[3]-0.5, 0, 0, 0, 180.0, 0, 0, 0.4, 0.4, 0.4, 207, 158, 25, 150, 0, 0, 0, 1)
                    if dist < 1 then
                        text3D(IniciarServico2[1], IniciarServico2[2], IniciarServico2[3]-0.5, '~y~[E] ~w~PARA DESMANCHAR O VEÍCULO')
                        if IsControlJustPressed(0,38) and vSERVER.CheckPerm() then
                            veh = CheckVeiculo(LocalDesmancharCarro2[1], LocalDesmancharCarro2[2], LocalDesmancharCarro2[3])
                            if veh then 
                                local VehPermitido, ClasseVeh = CheckClasse(veh)
                                placa = GetVehicleNumberPlateText(veh)
                                nomeCarro = GetDisplayNameFromVehicleModel(GetEntityModel(veh))
            ...
            elseif etapa2 == 1 then
                local dist = Vdist(pedCoords, LocalFerramentas2[1], LocalFerramentas2[2], LocalFerramentas2[3])
                if dist < 10 then
                    DrawMarker(21, LocalFerramentas2[1], LocalFerramentas2[2], LocalFerramentas2[3]-0.5, 0, 0, 0, 180.0, 0, 0, 0.4, 0.4, 0.4, 207, 158, 25, 150, 0, 0, 0, 1)
                    if dist < 1 then
                        text3D(LocalFerramentas2[1], LocalFerramentas2[2], LocalFerramentas2[3]-0.5, '~y~[E] ~w~PARA PEGAR AS ~y~FERRAMENTAS')
                        if IsControlJustPressed(0,38) then
                            if TipoVeh == 'carro' then
                                PosVeh['Porta_Direita'] = GetWorldPositionOfEntityBone(veh, GetEntityBoneIndexByName(veh,"handle_dside_f"))
                                PosVeh['Porta_Esquerda'] = GetWorldPositionOfEntityBone(veh, GetEntityBoneIndexByName(veh,"handle_pside_f"))
                            ...
                            FreezeEntityPosition(ped, true)
                            SetEntityHeading(ped, LocalFerramentas2[4])
                            vRP.playAnim(false, {{"amb@medic@standing@kneel@idle_a", "idle_a"}}, true)
                            TriggerEvent('progress', 5000, 'PEGANDO FERRAMENTAS')
            ...
            elseif etapa2 == 2 then

                if qtdPecasRemovidas == PecasVeh then
                    etapa2 = 3
                    -- print('Tudo desmontado, passando proxima etapa2')
                    TriggerEvent('Notify', 'sucesso', 'VEÍCULO DESMANCHADO COM SUCESSO! Vá até a bancada e anuncie o chassi do veículo.')
                end
            ...
            elseif etapa2 == 3 then
                local dist = Vdist(pedCoords, AnuncioChassi2[1], AnuncioChassi2[2], AnuncioChassi2[3])
                if dist < 10 then
                    DrawMarker(21, AnuncioChassi2[1], AnuncioChassi2[2], AnuncioChassi2[3]-0.5, 0, 0, 0, 180.0, 0, 0, 0.4, 0.4, 0.4, 207, 158, 25, 150, 0, 0, 0, 1)
                    if dist < 1 then
                        text3D(AnuncioChassi2[1], AnuncioChassi2[2], AnuncioChassi2[3]-0.5, '~y~[E] ~w~PARA ANUNCIAR O ~y~CHASSI')
                        if IsControlJustPressed(0,38) then
                            FreezeEntityPosition(ped, true)
                            SetEntityCoords(ped, AnuncioChassi2[1], AnuncioChassi2[2], AnuncioChassi2[3]-0.97)
                            SetEntityHeading(ped, AnuncioChassi2[4])
                            vRP.playAnim(false, {{"anim@heists@prison_heistig1_p1_guard_checks_bus", "loop"}}, true)
                            TriggerEvent('progress', 5000, 'ANUNCIANDO CHASSI DO VEÍCULO')
                            Wait(5000)
                            FreezeEntityPosition(ped, false)
