...
const updateGarages = () => {
        $.post('http://nation_garages/myVehicles', JSON.stringify({}), (data) => {
				...
				<div class="item" data-typecar='${item.type}' data-status='${item.status}' data-ipva='${item.ipva}' data-name="${item.name}" data-item-name="${item.name}" data-name2="${item.name2}" data-engine="${item.engine}" data-body="${item.body}" data-fuel="${item.fuel}">
					...
					<div id="backgrounded-car" style="background-image: url(http://102.165.46.151/images/vehicles/${item.name}.png);"></div>
				</div>
			...
			$("#mostraFoto").css("background-image",`url(http://102.165.46.151/images/vehicles/${$(vehicles[0]).data("name")}.png)`)
...
$(document).on('click','.item',function(){
	...
	$("#car-selected").fadeOut(0).delay(400).css("background-image",`url(http://102.165.46.151/images/vehicles/${$(this).data("name")}.png)`).fadeIn(400);
