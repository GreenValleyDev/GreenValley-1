...
const updateGarages = () => {
        $.post('http://nation_garages/myVehicles', JSON.stringify({}), (data) => {
				...
				<div class="item" data-typecar='${item.type}' data-status='${item.status}' data-ipva='${item.ipva}' data-name="${item.name}" data-item-name="${item.name}" data-name2="${item.name2}" data-engine="${item.engine}" data-body="${item.body}" data-fuel="${item.fuel}">
					...
					<div id="backgrounded-car" style="background-image: url(http://191.96.225.126/images/veiculos/${item.name}.png);"></div>
				</div>
			...
			$("#mostraFoto").css("background-image",`url(http://191.96.225.126/images/veiculos/${$(vehicles[0]).data("name")}.png)`)
...
$(document).on('click','.item',function(){
	...
	$("#car-selected").fadeOut(0).delay(400).css("background-image",`url(http://191.96.225.126/images/veiculos/${$(this).data("name")}.png)`).fadeIn(400);
