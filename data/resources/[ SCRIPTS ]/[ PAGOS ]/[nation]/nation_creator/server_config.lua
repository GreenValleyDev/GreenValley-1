...
function playerSpawn(user_id, source, first_spawn)
    if first_spawn then
        ...
        TriggerEvent("tkzin:resetdimensao")
        -- vRP.setHunger(user_id,-400)
        -- vRP.setThirst(user_id,-400)
        -- setPlayerTattoos() ---adicionei
	end
end
...
function processSpawnController(source,char,user_id)
	...
	if char then
        ...
        if data and data ~= "" then 
            ...
            vRP.setHunger(user_id,-400)
			vRP.setThirst(user_id,-400)
            TriggerEvent("tkzin:setardimensao")
        end
	end
end


RegisterServerEvent("tkzin:setardimensao")
AddEventHandler("tkzin:setardimensao",function(id)
    local source = source
    local user_id = id or vRP.getUserId(source)
    if user_id then
...
RegisterServerEvent("tkzin:resetdimensao")
AddEventHandler("tkzin:resetdimensao",function()
	local source = source
    local user_id = id or vRP.getUserId(source)
    if user_id then
...
RegisterCommand('resetplayer',function(source, args) -- COMANDO DE ADMIN PARA RESETAR PERSONAGEM
    if func.checkPermission({"manager.permissao", "administrador.permissao", "Admin"}, source) then
        if args[1] then 
...
RegisterCommand('spawn',function(source) -- COMANDO DE ADMIN PARA SIMULAR O SPAWN
    if func.checkPermission({"manager.permissao", "administrador.permissao", "Admin"}, source) or not vRP.getUserId(source) then
        if multiCharacter then
