...
--[[     vRP._prepare("getVehicles", "SELECT * FROM nation_concessionaria")
    ...
    Citizen.CreateThread(function()
        local vehs = vRP.query("getVehicles")--
        for i,veh in ipairs(vehs) do 
            vRP.execute("nation_conce/addVehicle", { vehicle = veh.modelo, estoque = 99 } )
        end
    end)
 ]]
...
RegisterCommand("conceestoque",function(source,args)
    local user_id = vRP.getUserId(source)
    if vRP.hasPermission(user_id,config.updateconce_permission) then
        if parseInt(args[1]) > 0 then
	        ...
	        for k,v in pairs(vehicles) do
                local tipo = v.tipo
                if tipo == "carros" or tipo == "motos" then
                    local isVehicleInConce = #vRP.query("nation_conce/isVehicleInConce",{ vehicle = k }) > 0
                    if isVehicleInConce then
                        vRP.execute("nation_conce/addEstoque", { vehicle = k, estoque = parseInt(args[1]) })
                    else
                        vRP.execute("nation_conce/addVehicle", { vehicle = k, estoque = parseInt(args[1]) })
                    end
                end
            end
        else
            TriggerClientEvent("Notify",source,"importante","Digite a quantidade de estoque dos carros",5000)
        end
    end
end)
