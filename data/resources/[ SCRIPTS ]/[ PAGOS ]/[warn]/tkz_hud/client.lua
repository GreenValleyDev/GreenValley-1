...
tkz = Tunnel.getInterface(GetCurrentResourceName())
...
AddEventHandler("nation_hud:setVoipMode",function(voiceMode)
	talkRange = voiceMode
end)
    	...
    	if IsPedInAnyVehicle(ped) then
     		local vehicle = GetVehiclePedIsUsing(ped)
      		...
      		if engine and not IsPedOnAnyBike(ped) and not IsPedInAnyHeli(ped) and not IsPedInAnyPlane(ped) then
				...
				TriggerEvent('vrp_sound:source','sem-cinto', 0.3)
				-- TriggerEvent('Notify','importante',"Você esta sem cinto.")
				end
      		end
    	end
	end
