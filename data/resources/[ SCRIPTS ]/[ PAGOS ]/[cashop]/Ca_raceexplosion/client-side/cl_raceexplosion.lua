...
vRPEXR = Tunnel.getInterface("Ca_raceexplosion")
-----------------------------------------------------------------------------------------------------------------------------------------
-- VARIAVEIS
-----------------------------------------------------------------------------------------------------------------------------------------
local blips = false
...
local CoordenadaZ = 29.29    --407.52, -1640.40, 29.29
   -- Antiga Coordenada
					...
					if distance <= 5.9 then
						...
						if IsControlJustPressed(0,38) then
							...
							if explosive >= 70 then
								...
								TriggerEvent("Notify","vermelho","Você começou uma corrida <b>Explosiva</b>, não saia do veículo e termine<br>no tempo estimado, ou então sei veículo vai explodir com você dentro.",10000)
							end
						end
					end
				end
					...
					if distance <= 15.1 then
						...
						if racepos == #races[racepoint] then
							...
							if explosive >= 70 then
								...
								vRPEXR.removeBombRace()
								vRPEXR.paymentCheck(racepoint,2)
							else
								vRPEXR.paymentCheck(racepoint,1)
							end
						else
							racepos = racepos + 1
							Gold8CreaterBliper(races,racepoint,racepos)
...
function Gold8CreaterBliper(races,racepoint,racepos)
...
RegisterNetEvent("races:defuse")
AddEventHandler("races:defuse",function()
	inrace = false
	SetVehicleNumberPlateText(GetPlayersLastVehicle(),PlateIndex)
	PlateIndex = nil
	RemoveBlip(blips)
