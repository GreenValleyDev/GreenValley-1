...
vCLIENT = Tunnel.getInterface("fw_marketplace")
...
vRP.prepare('fw_marketplace/getMyAds', 'SELECT * FROM fw_marketplace_items WHERE author_identifier = @author_identifier')


src.getMyAds = function()
	local _source = source
	local xPlayer = vRP.getUserId(_source)
	local data = vRP.query('fw_marketplace/getMyAds', {author_identifier=xPlayer})
	-- print(json.encode(data))
	return data
end


vRP.prepare('fw_marketplace/getItemsDB', 'SELECT * FROM fw_marketplace_items ORDER BY id ASC')

src.getItemAds = function()
	local _source = source
	local xPlayer = vRP.getUserId(_source)

	-- MySQL.Async.fetchAll('SELECT * FROM fw_marketplace_vehicles ORDER BY id ASC', {
	-- }, function(veh) 
		-- MySQL.Async.fetchAll('SELECT * FROM fw_marketplace_items ORDER BY id ASC', {
		-- }, function(items) 
			-- MySQL.Async.fetchAll('SELECT * FROM fw_marketplace_blackmarket ORDER BY id ASC', {
			-- }, function(blackmarket) 
				-- local veh = {}
				-- local blackmarket = {}

				-- local data = vRP.query('fw_marketplace/getItemsDB')
				local items = vRP.query('fw_marketplace/getItemsDB', {user_id=user_id})
				-- print(json.encode(data))
	...
	-- }, function(veh) 
		...
		-- }, function(items) 
			...
			-- }, function(blackmarket) 
				...
				-- local data = vRP.query('fw_marketplace/getItemsDB')
				local items = vRP.query('fw_marketplace/getItemsDB')
				-- print(json.encode(data))
...
-- RegisterServerEvent("fw_marketplace:addVehicle")
-- AddEventHandler("fw_marketplace:addVehicle", function(vehicle, price, desc, phone_number)
-- 	local _source = source
-- 	local xPlayer = ESX.GetPlayerFromId(_source)
...
-- 	MySQL.Async.insert('INSERT INTO fw_marketplace_vehicles (item_id, plate, label, author_identifier, author_name, phone_number, description, price, start_date) VALUES (@item_id, @plate, @label, @author_identifier, @author_name, @phone_number, @description, @price, @start_date)', {
-- 		['@item_id'] = id,
-- 		['@plate'] = plate,
-- 		['@label'] = vehicle.name,
...
-- 	}, function(result)
-- 		TriggerClientEvent('fw_marketplace:updateVehiclesDropdown', xPlayer.source)
-- 		TriggerClientEvent('fw_marketplace:updateVehicles', xPlayer.source)
-- 		TriggerClientEvent('fw_marketplace:updateMyAdsTable', xPlayer.source)
...
-- end)
	...
	vRP.prepare('fw_marketplace/SetDBItems', 'INSERT INTO fw_marketplace_items (item_id, label, amount, author_identifier, author_name, phone_number, description, price, start_date) VALUES (@item_id, @label, @amount, @author_identifier, @author_name, @phone_number, @description, @price, @start_date)')

RegisterServerEvent("fw_marketplace:addItem")
AddEventHandler("fw_marketplace:addItem", function(item, amount, price, desc, phone_number)
	...
	-- if true then
		...
		-- MySQL.Async.insert('INSERT INTO fw_marketplace_items (item_id, label, amount, author_identifier, author_name, phone_number, description, price, start_date) VALUES (@item_id, @label, @amount, @author_identifier, @author_name, @phone_number, @description, @price, @start_date)', {
		-- 	['@item_id'] = item.id,
		...
		vRP.execute('fw_marketplace/SetDBItems', {
			item_id = item.id,
			label = vRP.itemNameList(item.id),
			amount = amount,
			author_identifier = xPlayer,
		...
		-- print('teste')
			...
			TriggerClientEvent('fw_marketplace:updateItemsDropdown', _source)
			TriggerClientEvent('fw_marketplace:updateItems', _source)
			TriggerClientEvent('fw_marketplace:updateMyAdsTable', _source)
			vRP.tryGetInventoryItem(xPlayer, item.id, amount)
			TriggerClientEvent('Notify', _source, "sucesso", "Agora você está vendendo "..vRP.itemNameList(item.id).." ("..amount..")", 5000)
...
-- RegisterServerEvent("fw_marketplace:addBlackmarket")
-- AddEventHandler("fw_marketplace:addBlackmarket", function(item, price, desc, phone_number, amount)
-- 	local _source = source
-- 	local xPlayer = ESX.GetPlayerFromId(_source)

-- 	if item.type == "weapon" and xPlayer.hasWeapon(item.id) and amount <= item.count then
-- 		MySQL.Async.insert('INSERT INTO fw_marketplace_blackmarket (item_id, label, type, amount, author_identifier, author_name, phone_number, description, price, start_date) VALUES (@item_id, @label, @type, @amount, @author_identifier, @author_name, @phone_number, @description, @price, @start_date)', {
-- 			['@item_id'] = item.id,
-- 			['@label'] = item.label,
-- 			['@type'] = item.type,
-- 			['@amount'] = amount,
...
-- 			['@start_date'] = os.date("%d/%m - %H:%M"),
-- 		}, function(result)
-- 			TriggerClientEvent('fw_marketplace:updateBlackmarketDropdown', xPlayer.source)
-- 			TriggerClientEvent('fw_marketplace:updateBlackmarket', xPlayer.source)
-- 			TriggerClientEvent('fw_marketplace:updateMyAdsTable', xPlayer.source)
...
-- RegisterServerCallback('fw_marketplace:getVehicle', function(source, cb, id)
-- 	local _source = source
-- 	local xPlayer = ESX.GetPlayerFromId(_source)
-- 	MySQL.Async.fetchAll('SELECT * FROM fw_marketplace_vehicles WHERE item_id = @item_id AND sold = false', {
-- 		['@item_id'] = id,
-- 	}, function(veh)
-- 		if veh[1] ~= nil then
-- 			cb(veh)
...
src.getItem = function(source, cb, id, item)
	...
	MySQL.Async.fetchAll('SELECT * FROM fw_marketplace_items WHERE id = @id AND item_id = @item_id AND sold = false', {
		['@id'] = id,
		['@item_id'] = item,
	}, function(item)
		if item[1] ~= nil then
			...
			TriggerClientEvent('fw_marketplace:updateItems', xPlayer.source)
			TriggerClientEvent('okokNotify:Alert', xPlayer.source, "MARKET", "This item is no longer for sale", 5000, 'error')
		end
	end)
end

-- RegisterServerCallback('fw_marketplace:getBlackmarket', function(source, cb, id, blackmarket)
-- 	local _source = source
-- 	local xPlayer = ESX.GetPlayerFromId(_source)

-- 	MySQL.Async.fetchAll('SELECT * FROM fw_marketplace_blackmarket WHERE id = @id AND item_id = @item_id AND sold = false', {
-- 		['@id'] = id,
-- 		['@item_id'] = blackmarket,
-- 	}, function(blackmarket)
...
-- RegisterServerEvent("fw_marketplace:buyVehicle")
-- AddEventHandler("fw_marketplace:buyVehicle", function(veh)
-- 	local _source = source
-- 	local xPlayer = ESX.GetPlayerFromId(_source)
...
-- 	if money >= price then
-- 		MySQL.Async.execute('UPDATE fw_marketplace_vehicles SET sold = 1 WHERE plate = @plate AND sold = 0', {['@plate'] = veh[1].plate},
...
RegisterServerEvent("fw_marketplace:buyItem")
AddEventHandler("fw_marketplace:buyItem", function(item_id, item, value, amountVal, playerTarget)
	local _source = source
	local xPlayer = vRP.getUserId(_source)
	...
	if xPlayer == parseInt(playerTarget) then
		TriggerClientEvent("Notify", _source, "negado", "Você não pode comprar seu próprio item.")
		return
	end
	...
	if money >= price then
		MySQL.Async.execute('UPDATE fw_marketplace_items SET sold = 1 WHERE id = @id AND item_id = @item_id AND sold = 0', {
			['@id'] = item_id,
			['@item_id'] = item,
		},function (rowsChanged)
			if rowsChanged > 0 then
				...
				TriggerClientEvent('Notify', _source, "sucesso", "Você comprou o item "..vRP.itemNameList(item).." (x"..amount..")", 5000)
				TriggerClientEvent('fw_marketplace:updateItemsDropdown', _source)
				TriggerClientEvent('fw_marketplace:updateItems', _source)
				-- if xTarget ~= nil then
		...
		TriggerClientEvent('Notify', _source, "negado", "Você não tem dinheiro para comprar este item", 5000)
	end
end)



-- RegisterServerEvent("fw_marketplace:buyBlackmarket")
-- AddEventHandler("fw_marketplace:buyBlackmarket", function(blackmarket)
-- 	local _source = source
-- 	local xPlayer = ESX.GetPlayerFromId(_source)
...
-- 			MySQL.Async.execute('UPDATE fw_marketplace_blackmarket SET sold = 1 WHERE id = @id AND item_id = @item_id AND sold = 0', {
-- 					['@id'] = blackmarket[1].id,
...
RegisterServerEvent("fw_marketplace:removeMyAd")
AddEventHandler("fw_marketplace:removeMyAd", function(item)
	local _source = source
	local xPlayer = vRP.getUserId(_source)

	if item.plate then
		-- MySQL.Async.fetchAll('SELECT * FROM fw_marketplace_vehicles WHERE item_id = @item_id AND id = @id', {
		-- 	['@item_id'] = item.item_id,
		-- 	['@id'] = item.id,
		-- }, function(veh)
		-- 	MySQL.Async.execute('DELETE FROM fw_marketplace_vehicles WHERE item_id = @item_id AND id = @id', {
		-- 		['@id'] = veh[1].id,
	...
	elseif item.type then
		-- MySQL.Async.fetchAll('SELECT * FROM fw_marketplace_blackmarket WHERE item_id = @item_id AND id = @id', {
		-- 	['@item_id'] = item.item_id,
		-- 	['@id'] = item.id,
		-- }, function(blackmarket)
		-- 	local canCarry = true
		...
		-- 	if canCarry then
		-- 		MySQL.Async.execute('DELETE FROM fw_marketplace_blackmarket WHERE item_id = @item_id AND id = @id', {
		-- 			['@id'] = blackmarket[1].id,
		-- 			['@item_id'] = blackmarket[1].item_id,
		-- 		},function (rowDeleted)
		-- 			if rowDeleted > 0 then
		...
		MySQL.Async.fetchAll('SELECT * FROM fw_marketplace_items WHERE item_id = @item_id AND id = @id', {
			['@item_id'] = item.item_id,
			['@id'] = item.id,
		}, function(items)
			-- if xPlayer.canCarryItem(items[1].item_id, tonumber(items[1].amount)) then
				MySQL.Async.execute('DELETE FROM fw_marketplace_items WHERE item_id = @item_id AND id = @id', {
					['@id'] = items[1].id,
					...
					if rowDeleted > 0 then
						if items[1].sold then
							...
							TriggerClientEvent('Notify', _source, "sucesso", "Você retirou "..items[1].price.." R$", 5000)
							-- if Webhook ~= '' then
							-- 	local identifierlist = ExtractIdentifiers(xPlayer.source)
							-- 	local data = {
							-- 		playerid = xPlayer.source,
							...
							TriggerClientEvent('fw_marketplace:updateItemsDropdown', _source)
							TriggerClientEvent('fw_marketplace:closeMarket', _source)
							TriggerClientEvent('Notify', _source, "sucesso", "Você cancelou o anúncio "..items[1].label.." (x"..items[1].amount..")", 5000)
							-- if Webhook ~= '' then
							-- 	local identifierlist = ExtractIdentifiers(xPlayer.source)
							-- 	local data = {
							-- 		playerid = xPlayer.source,
	...
	TriggerClientEvent('fw_marketplace:updateMyAds', _source)
end)
