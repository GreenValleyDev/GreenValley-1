...
$(function() {
    window.addEventListener('message', function(event) {
        if (event.data.type == "enableui") {
			...
			if (event.data.enable) {			
				...
				$hit.textContent = '[Novo jogo] - Multiplicador x2';
				$stay.textContent = '[Aumentar] +100';
				$score.textContent = '[Diminuir] -100';
	...
	$hit.addEventListener('click', function () {
		...
		if ($hit.textContent.includes("[Novo jogo] - Multiplicador x2") && BlackJack.bet >= 100) {
			$.post("http://blackjack/card", JSON.stringify({}));
			$('#ownedPoints').empty().append(0);
			...
			$.post("http://blackjack/card", JSON.stringify({}));
			// NEW GAME
			$hit.textContent = "[RECEBER CARTA]";
			coins = coins - BlackJack.bet;
			$('#ownedCoins').empty().append(coins);
			$('#ownedBet').empty().append(BlackJack.bet);
			$stay.textContent = "[PARAR]";
			$score.textContent = "O oponente tem 0 pontos;";
			$.post("http://blackjack/card", JSON.stringify({}));
			...
			var prom = new Promise(function(res, rej) {
				...
				$score.textContent = "O oponente tem " + BlackJack.getScore(BlackJack.dealerHand.slice(0, 1)) + " pontos.";
			});
		} else if (BlackJack.myTurn) {
			$stay.disabled = 'disabled';
			queActions(1);
			BlackJack.PlayerHit();
			$.post("http://blackjack/card", JSON.stringify({}));
			var prom = new Promise(function(res, rej) {
				...
				if (BlackJack.didBust(BlackJack.hand)) {
					...
					BlackJack.Message('Você ~r~perdeu~w~ com ' + BlackJack.getScore(BlackJack.hand) + ' pontos.');
					$hit.textContent = '[Novo jogo] - Multiplicador x2';
					$stay.textContent = '[Aumentar] +100';
					$score.textContent = '[Diminuir] -100';
					$('#ownedPoints').empty().append(0);
	...
	$stay.addEventListener('click', function () {
		...
		if ($stay.textContent.includes("[Aumentar] +100")) {
			...
			$score.textContent = "O oponente tem " + BlackJack.getScore(BlackJack.dealerHand) + " pontos.";
			...
			if (BlackJack.getScore(BlackJack.dealerHand) > BlackJack.getScore(BlackJack.hand)) {
				...
				$score.textContent = '[Diminuir] -100';
				BlackJack.Lost();
				BlackJack.Message('Você ~r~perdeu~w~ com ' + BlackJack.getScore(BlackJack.hand) + ' pontos contra ' + BlackJack.getScore(BlackJack.dealerHand) + ".");
				$('#ownedBet').empty().append(100);
				queActions(0);
				...
				$score.textContent = '[Diminuir] -100';
				BlackJack.Tie();
				BlackJack.Message('Você ~y~empatou~w~ com ' + BlackJack.getScore(BlackJack.hand) + ' pontos contra ' + BlackJack.getScore(BlackJack.dealerHand) + ".");
				$('#ownedBet').empty().append(100);
				queActions(0);
				...
				$score.textContent = '[Diminuir] -100';
				BlackJack.Win();
				BlackJack.Message('Você ~g~ganhou~w~ com ' + BlackJack.getScore(BlackJack.hand) + ' pontos contra ' + BlackJack.getScore(BlackJack.dealerHand) + ".");
				$('#ownedPoints').empty().append(0);
				$('#ownedBet').empty().append(100);
				queActions(0);
			} else {
	...
	$score.addEventListener('click', function () {
		...
		if ($score.textContent.includes("[Diminuir] -100")) {
			$.post("http://blackjack/bet", JSON.stringify({}));
