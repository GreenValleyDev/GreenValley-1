	...
	if user_id then
		if vRPclient.getHealth(source) >= 102 then
			...
			if nplayer then
				local nuser_id = vRP.getUserId(nplayer)
				local identitynu = vRP.getUserIdentity(nuser_id)
				if nuser_id and vRPclient.getHealth(nplayer) >= 102 and not vCLIENT.inVehicle(nplayer) and not vCLIENT.inVehicle(source) then
					...
					if check then
						...
						vCLIENT.toggleCarry(nplayer,source)
						vRPclient._playAnim(source,false,{config.inspectAnim},true)
						vRPclient._playAnim(nplayer,false,{config.nuInspectAnim},true)
						TriggerClientEvent("progress",source,5000,"revistando")
						SetTimeout(5000,function()

							local message = ""
							message = message.."MOCHILA [ 3"..string.format("%.2f",vRP.getInventoryWeight(nuser_id)).."kg / "..string.format("%.2f",vRP.getInventoryMaxWeight(nuser_id)).."kg ] <br>"
							...
							if data and data.inventory then
								for k,v in pairs(data.inventory) do
									message = message.." "..vRP.format(parseInt(v.amount)).."x "..vRP.itemNameList(v.item).." <br>"
									-- TriggerClientEvent('chatMessage',source,"",{},"     "..vRP.format(parseInt(v.amount)).."x "..vRP.itemNameList(v.item))
								end
							end
							message = message.."EQUIPADO<br>"
							-- TriggerClientEvent('chatMessage',source,"",{},"^4- -  ^5E Q U I P A D O^4  - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -")
							for k,v in pairs(weapons) do
								if v.ammo < 1 then
									message = message.." 1x "..vRP.itemNameList("wbody|"..k).." <br>"
									-- TriggerClientEvent('chatMessage',source,"",{},"     1x "..vRP.itemNameList("wbody|"..k))
								else
									message = message.." 1x "..vRP.itemNameList("wbody|"..k).." | "..vRP.format(parseInt(v.ammo)).."x Munições <br>"
							...
							TriggerClientEvent('carregar2',nplayer,source)
							vCLIENT.toggleCarry(nplayer,source)
							message = message.." $"..vRP.format(parseInt(money)).." Dólares"
							TriggerClientEvent("Notify",source,"aviso",message,15000)
							-- TriggerClientEvent('chatMessage',source,"",{},"     $"..vRP.format(parseInt(money)).." Dólares")
							PerformHttpRequest(config.webhookInspect, function(err, text, headers) end, 'POST', json.encode({
								embeds = {
									{
