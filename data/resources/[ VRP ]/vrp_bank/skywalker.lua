...
RegisterCommand(config.fines,function(source,args,rawCommand)
	...
	if user_id then
		...
		elseif args[1] == config.pay then
			...
			if tonumber(valorpay) <= 0 then
				TriggerClientEvent("Notify",source,"negado","Você não pode colocar um vallor menor que 0.",8000)
				return
			end
					...
					PerformHttpRequest(config.logBankFines, function(err, text, headers) end, 'POST', json.encode({
						embeds = {
							{ 	------------------------------------------------------------
								...
								thumbnail = {
									url = "https://cdn.discordapp.com/attachments/908814432019292260/908820312060215306/c9593eb5-f83e-47ef-a90d-e5d9c84694eb.png"
								}, 
								fields = {
									{ 
										name = "**IDENTIFICAÇÃO:**",
								...
								footer = { 
									...
									icon_url = "https://cdn.discordapp.com/attachments/908814432019292260/908820312060215306/c9593eb5-f83e-47ef-a90d-e5d9c84694eb.png"
								},
								color = 4402032 
							}
						}
...
RegisterServerEvent('bank:depositar20')
AddEventHandler('bank:depositar20', function(amount)
...
RegisterServerEvent('bank:sacar20')
AddEventHandler('bank:sacar20', function(amount)
	local _source = source
	local user_id = vRP.getUserId(_source)
	local identity = vRP.getUserIdentity(user_id)
...
RegisterServerEvent('bank:pagar20')
AddEventHandler('bank:pagar20', function(amount)
	local _source = source
	local user_id = vRP.getUserId(_source)
	local bank = vRP.getBankMoney(user_id)
	local identity = vRP.getUserIdentity(user_id)
...
RegisterServerEvent('bank:balance20')
AddEventHandler('bank:balance20', function()
	local _source = source
	local user_id = vRP.getUserId(_source)
	local getbankmoney = vRP.getBankMoney(user_id)
...
RegisterServerEvent('bank:transferir20')
AddEventHandler('bank:transferir20', function(to,amountt)
	local _source = source
	local user_id = vRP.getUserId(_source)
	local identity = vRP.getUserIdentity(user_id)
