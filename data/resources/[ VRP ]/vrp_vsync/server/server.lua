...
yRP = {}
Tunnel.bindInterface("vrp_vsync",yRP)
...
RegisterCommand("time", function(source)
    ...
    if PermissionsCheck(_source) then
        TriggerClientEvent('cd_easytime:time', _source, {weather = Weather, time = Hours, mins = Mins, dynamic = Dynamic, blackout = Blackout, freeze = FreezeTime})
    else
...
RegisterCommand("boladeneve",function(source,args,rawCommand)
...
Citizen.CreateThread(function()
	while true do
		...
		TriggerClientEvent('vrp_vsync:syncTimers',-1,Hours,Mins)
	end
end)
...
Citizen.CreateThread(function()
    while true do
        ...
        if Dynamic and not shouldstop then
...
AddEventHandler("vRP:playerSpawn",function(user_id,source)
    if Config.Natal then
	    TriggerClientEvent("cd_easytime:natal",source)
    end
    if Config.ForceTimeClear then
	    TriggerClientEvent("cd_easytime:clear",source)
    end
end) 
